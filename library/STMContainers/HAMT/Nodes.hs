...
stream :: (MonadTrans t, MonadPlus (t STM)) => Level.Level -> Nodes e -> t STM e
stream l =
  lift . readTVar >=> fromFoldable >=> \case
    Nodes n -> stream (Level.succ l) n
    Leaf _ e -> return e
    Leaves _ a -> fromFoldable a
  where
    fromFoldable :: (Foldable f, Alternative m) => f a -> m a
    fromFoldable = foldr (\x xs -> pure x <|> xs) empty
